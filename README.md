# Sales Assistant Lead Scoring App

This web app helps sales reps evaluate property-management and apartment leads. A rep can enter a lead manually or connect Gmail for inbound inquiries. The backend enriches the lead with public data, scores the opportunity, writes category-level insights, and drafts an outreach or reply email.

## Project Structure

- `backend/`: Node.js API for lead extraction, enrichment, scoring, Gmail OAuth, AI agents, and draft email generation.
- `frontend/`: React/Vite web app for manual lead input, Gmail connection, lead queue, score details, insights, and draft email UI.
- `n8n/`: Optional n8n workflow template. The current app can run without n8n because the frontend talks directly to the backend through `/api`.
- `docker-compose.yml`: Optional local n8n + Postgres setup.
...
At minimum, the app can run with fallback data. For better enrichment, fill these in `backend/.env`:
...
GMAIL_REDIRECT_URI=http://localhost:8787/api/gmail/oauth/callback
```

Do not commit `backend/.env`, `frontend/.env`, or `backend/.gmail-token.json`. They are ignored by `.gitignore`.
...
Vite proxies `/api` requests to `http://localhost:8787`, so the frontend and backend work together locally.

### 5. Use The App

1. Open `http://localhost:5173`.
2. Enter a lead manually with company/property name, email, and property address.
3. Click the enrich/score button.
4. Review the lead score, category insights, overall analysis, and draft outreach email.
5. Optional: connect Gmail from the Gmail tab to turn inbound emails into scored leads.

## Optional Gmail Setup

To connect Gmail, create a Google OAuth app and make sure the authorized redirect URI exactly matches:
...
GMAIL_REDIRECT_URI=http://localhost:8787/api/gmail/oauth/callback
GMAIL_SEARCH_QUERY=in:inbox newer_than:7d -category:social -category:promotions
```

After starting the backend and frontend, use the Gmail tab in the app to connect the inbox.

## Optional n8n Setup

The app does not require n8n for the normal local workflow. If you want to test the included n8n template:
...
Import `n8n/workflow.json` into n8n. The included workflow normalizes Gmail, Google Sheets, or webhook inputs and calls the backend enrichment endpoint.
